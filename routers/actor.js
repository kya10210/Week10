...
module.exports = {
    ...
    deleteAndAllMovies: function(req,res){
        Actor.findOneAndRemove({ _id: req.params.id }, function (err,doc) {
            if (err) return res.status(400).json(err);
            console.log(doc);
            
            Movie.deleteMany({'actors': doc},function(err,doc){});
            res.json();
        });
    },

    //remove a movie from the list of movies of an actor
    removeMoive: function(req,res){
            ...
            Movie.findOne({_id: req.params.movieid},function(err,movie){
                ...
                if (movie){
                    ...
                    for(var i = 0;i<actor.movies.length;i++){
                        if(actor.movies[i]._id == req.params.movieid){
    ...
    incrementAge:function(req, res){
        ...
        Actor.updateMany({'bYear':{$lte:1969}},{$inc : {'bYear' : 4}}).exec(function (err,actor){
                if (err) return res.status(400).json(err);
                if (!actor) return res.status(404).json();
                res.json(actor);
                })
